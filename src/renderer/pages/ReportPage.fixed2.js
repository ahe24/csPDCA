...
const ReportPage = () => {
  ...
  // Function to get the display date for a task
  const getDisplayDate = (task) => {
    // For all-day events, we need to parse the date directly from the string
    // to avoid timezone issues
    if (task.is_all_day === 1) {
      const dateParts = task.start_datetime.split('T')[0].split('-');
      return new Date(dateParts[0], parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));
    } else {
      return new Date(task.start_datetime);
    }
  };
                    ...
                    <tbody>
                      {[...report.tasks]
                        ...
                        .map((task) => {
                          ...
                          return (
                            <tr key={task.uuid}>
                              <td>{displayDate.toLocaleDateString('ko-KR', { month: 'numeric', day: 'numeric' })}</td>
                              <td>{task.title}</td>
                              ...
                              <td>{task.is_internal_work === 1 ? '내부' : task.external_location || '외부'}</td>
                              <td>{task.do_text || '-'}</td>
